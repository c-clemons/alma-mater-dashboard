...
def show():
    ...
    with col4:
        ...
        avg_monthly_revenue = monthly_df['Total Revenue'].mean()
        avg_monthly_opex = monthly_df['Total OpEx'].mean()
        avg_monthly_cogs = monthly_df['Total COGS'].mean()
        avg_monthly_burn = (avg_monthly_opex + avg_monthly_cogs) - avg_monthly_revenue
        
        if avg_monthly_burn > 0:
            days_of_cash = (net_cash / (avg_monthly_burn / 30))
            st.metric(
                "Days of Cash",
