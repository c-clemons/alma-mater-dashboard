...
def load_monthly_actuals_2025():
    ...
    opex_monthly = [total_opex_annual / 12] * 12
...
def show():
    ...
    with tab2:
        ...
        for col in ['DTC Revenue', 'Wholesale Revenue', 'Total Revenue', 'DTC COGS', 'Wholesale COGS', 'Total COGS', 'Gross Profit', 'Team Costs', 'Other OpEx', 'Total OpEx', 'EBITDA']:
            display_df[col] = display_df[col].apply(lambda x: f"${x:,.0f}")
